...
# Parameters for Case 10
m, n = 3, 2
c = np.array([800, 500, 600])  # Capacities
p = np.array([
    [2, 4],  # Sparkling Water
    [3, 5],  # Iced Tea
    [4, 6]   # Fruit Juice
])  # Profit per unit
demand_means = np.array([600, 400])  # Mean demands
demand_stds = np.array([80, 50])     # Standard deviations of demands

...
# import instructor library if available
...
from .tools import BaseTool, Tool, FinalAnswerTool
from .utils import (
    AgentError,
    AgentExecutionError,
...
class MultiStepAgent(ABC):
    ...
    def run(
        ...
        if hasattr(self, 'input_schema') and self.input_schema is not None:
            # if input_schema is defined, structured_input must be provided
            if structured_input is None:
                ...
                # validate structured_input against input_schema
                try:
...
{str(additional_args)}."""
        ...
        # structured output validation and conversion
        if hasattr(self, 'output_schema') and self.output_schema is not None:
            try:
                output_str = str(output)
                try:
                    # use model_validate_json to parse the output string
                    output = self.output_schema.model_validate_json(output_str)
                except Exception:
                    # handle Python dictionary format (single quotes)
                    import ast
                    try:
                        # use ast.literal_eval safely parse Python dictionary format
                        parsed_dict = ast.literal_eval(output_str)
                        # convert to standard JSON string
                        import json as json_module
                        json_str = json_module.dumps(parsed_dict)
                        output = self.output_schema.model_validate_json(json_str)
                    except Exception as inner_e:
                        # if all attempts fail, raise error
                        raise ValueError(f"cannot parse output to {self.output_schema.__name__} format: {str(inner_e)}") from inner_e
            except Exception as e:
                error_msg = f"cannot parse output to {self.output_schema.__name__} format. original output: {output_str[:200]}...\nerror: {str(e)}"
...
class CodeAgent(MultiStepAgent):
    ...
    Args:
        ...
        **kwargs: Additional keyword arguments.
            ...
            <Added version="1.18.0"/>
        input_schema (`Any`, *optional*): Pydantic BaseModel subclass for validating structured input.
            If provided, agent will only accept structured input matching this schema.
        output_schema (`Any`, *optional*): Pydantic BaseModel subclass for validating structured output.
            If provided, agent will ensure output matches this schema or raise error.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # init output schema and input schema
        try:
            from pydantic import BaseModel
        except ImportError:
            self.logger.log(
                "Warning: Pydantic is required for structured input/output support but not installed.",
                level=LogLevel.WARNING
            )
            self.input_schema = None
            self.output_schema = None
        else:
            # validate schema type must be pydantic subclass
            if input_schema is not None and (not isinstance(input_schema, type) or not issubclass(input_schema, BaseModel)):
                raise TypeError("input_schema must be a Pydantic BaseModel subclass")
            if output_schema is not None and (not isinstance(output_schema, type) or not issubclass(output_schema, BaseModel)):
    ...
    def initialize_system_prompt(self) -> str:
        ...
        if self.output_schema:
            ...
            schema_description = self.output_schema.__doc__ or f"{schema_name}data structure model"
...
Your final answer must be outputted in the following format, ensuring compliance with the requirements of the {schema_name} model:

## Output Format
Please output in JSON format, ensuring all necessary fields are included:
{fields_info_str}

## Output Description
- Only output valid JSON objects, do not include any additional text
- Ensure all necessary fields have the correct values
- For numerical types, use the correct number format; for boolean types, use true/false
- Strictly follow JSON syntax, ensuring the output can be correctly parsed by Python's json.loads() function
"""

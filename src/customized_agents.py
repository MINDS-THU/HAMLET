...
class MultiStepAgent:
    ...
    Args:
        ...
        needs_final_answer (bool, *optional*): If False, skip the forced final_answer phase.
    """
    ...
    def run(
        ...
        terminal_tools: Optional[List[str]] = None,  # <-- NEW PARAM
    ):
        """
        ...
        Args:
            ...
            terminal_tools (`list[str]`, *optional*): A list of tool names. Whenever any of them is called, the reasoning loop will terminate.
...
{str(additional_args)}."""
        ...
        if stream:
            ...
            return self._run(task=self.task, max_steps=max_steps, images=images, terminal_tools=terminal_tools)
        # Outputs are returned only at the end. We only look at the last step.
        return deque(self._run(task=self.task, max_steps=max_steps, images=images, terminal_tools=terminal_tools), maxlen=1)[0]

    def _run(
        self, task: str, max_steps: int, images: List[str] | None = None, terminal_tools: Optional[List[str]] = None
    ) -> Generator[ActionStep | AgentType, None, None]:
        terminate = False
        self.step_number = 1
        while (not terminate) and self.step_number <= max_steps:
            step_start_time = time.time()
            memory_step = self._create_memory_step(step_start_time, images)
            try:
                terminate = self._execute_step(task, memory_step, terminal_tools)
    ...
    def _execute_step(self, task: str, memory_step: ActionStep, terminal_tools: Optional[List[str]] = None) -> Union[None, Any]:
        ...
        terminate = self.step(memory_step, terminal_tools)
        return terminate
    ...
    def step(self, memory_step: ActionStep, terminal_tools: Optional[List[str]] = None) -> bool:
        """To be implemented in children classes. Should return either True or False, indicating whether the task has been completed."""
        pass
    ...
    def save(self, output_dir: str, relative_path: Optional[str] = None):
        ...
        app_template = textwrap.dedent("""
            import yaml
            import os
            from src.gradio_ui import GradioUI, {{ class_name }}, {{ agent_dict['model']['class'] }}
    ...
    def from_folder(cls, folder: Union[str, Path], **kwargs):
        ...
        for managed_agent_name, managed_agent_class in agent_dict["managed_agents"].items():
            agent_cls = getattr(importlib.import_module("src.agents"), managed_agent_class)
        ...
        model_class: Model = getattr(importlib.import_module("src.models"), agent_dict["model"]["class"])
...
class CodeAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("src.prompts").joinpath("customized_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
    ...
    def step(self, memory_step: ActionStep, terminal_tools: Optional[List[str]] = None) -> bool:
        ...
        terminate = False
        try:
            output, execution_logs, terminate = self.python_executor(code_action, terminal_tools)
        ...
        execution_outputs_console += [
            Text(
                f"{('Out - Final step' if terminate else 'Out')}: {truncated_output}",
                style=(f"bold {YELLOW_HEX}" if terminate else ""),
            ),
        ]
        self.logger.log(Group(*execution_outputs_console), level=LogLevel.INFO)
...
class ToolCallingAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("src.prompts").joinpath("toolcalling_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
...
class BargainingAgent(MultiStepAgent):
    """
    In this agent, the tool calls will be formulated by the LLM in code format, then parsed and executed.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert planning_interval is None # no planning for bargaining agent
        self.additional_authorized_imports = additional_authorized_imports if additional_authorized_imports else []
        self.authorized_imports = list(set(BASE_BUILTIN_MODULES) | set(self.additional_authorized_imports))
        self.max_print_outputs_length = max_print_outputs_length
        prompt_templates = prompt_templates or yaml.safe_load(
    ...
    def initialize_system_prompt(self) -> str:
        system_prompt = populate_template(
            ...
            variables={
                "role": self.role,
                "item_name": self.scenario_data["item_name"],
                "buyer_item_description": self.scenario_data["buyer_item_description"],
                "seller_item_description": self.scenario_data["seller_item_description"],
                "listing_price": self.scenario_data["listing_price"],
                "buyer_bottomline_price": self.scenario_data["buyer_bottomline_price"],
                "seller_bottomline_price": self.scenario_data["seller_bottomline_price"],
                "max_round": self.scenario_data["max_round"],
                "tools": self.tools,
                "authorized_imports": (
                    "You can import from any package you want."
                    if "*" in self.authorized_imports

...
class GradioUI:
    ...
    def create_app(self):
        custom_css = """
        /* full‑height columns and VS Code‑style divider/header */
        .gr-block.gr-column {height: 100%;}
        .vscode-pane {
            border-left: 1px solid #e0e0e0;
            display:flex;                 /* stack explorer & preview */
            flex-direction:column;
            height:100%;
        }
        .vscode-header {background:#f6f6f6; font-weight:600; padding:4px 8px;}

        /* explorer fixed at 200 px */
        .vscode-pane .gr-file-explorer {flex:0 0 200px;}

        /* preview code editor fills remaining space */
        .vscode-pane .gr-code {
            flex:1 1 auto;
            width:100% !important;
        ...
        with gr.Blocks(theme="ocean", fill_height=True, css=custom_css) as demo:
            ...
            with gr.Row(scale=12):                       # full width minus sidebar
                ...
                with gr.Column(scale=5, min_width=350, elem_classes="vscode-pane"):
                    # header
                    gr.Markdown("### Files", elem_classes="vscode-header")
                    ...
                    file_explorer = gr.FileExplorer(
                        ...
                        interactive=True,
                        value=list_folder,
                        height=200,                     # top pane height
                    )

                    # header
                    gr.Markdown("### Preview", elem_classes="vscode-header")

                    preview_box = gr.Code(
                        label=None,
                        interactive=False,
                        elem_id="preview-pane",
                        elem_classes="preview-code",     #  ← NEW
                        lines=20,
                    )
                    # hook explorer ➜ preview
                    file_explorer.change(show_preview, [file_explorer], [preview_box])
